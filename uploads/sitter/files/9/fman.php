...
{ // no filesystem não podemos ter acentos
    $str = remove_acentos(trim($str));
    // Substitui caracteres reservados
    $str = str_replace('\\', '_', $str);
    $str = str_replace('/', '_', $str);
...
{ // no download podemos ter acentos
    $str = trim($str);
    // Substitui caracteres reservados
    $str = str_replace('\\', ' ', $str);
    $str = str_replace('/', ' ', $str);
...
function html_header($header = "")
{
    ...
    <style>
        ...
        .noselect {
            ...
            user-select: none; /* Non-backered version */
        }
        .btn {
...
class tree_fs
{
    ...
    public function __construct($base)
    {
        ...
        if (!$this->base) {
            fb_log('Base directory does not exist');
        }
    }
    protected function real($path)
    {
        ...
        if (!$temp) {
            fb_log('Path does not exist: ' . $path);
        }
        if ($this->base && strlen($this->base)) {
            if (strpos($temp, $this->base) !== 0) {
                fb_log('Path is not inside base (' . $this->base . '): ' . $temp);
            }
        }
        return $temp;
    }
    ...
    public function lst($id, $with_root = false)
    {
        ...
        if (!$lst) {
            fb_log('Could not list path: ' . $path);
        }
        $res = array();
        foreach ($lst as $item) {
...
?>
    ...
    <style>
        ...
        #tree .folder {
            background: url('<?php echo $fm_path_info["basename"]; ?>?action=99&filename=file_sprite.png') right bottom no-repeat;
        }

        #tree .file {
            background: url('<?php echo $fm_path_info["basename"]; ?>?action=99&filename=file_sprite.png') 0 0 no-repeat;
        }
...
{ // Does not provide output, could throw it to a file!
    if (is_string($args)) $args = array($args);
    $envs = array();
    ...
    function extract_files()
    {
        ...
        if ($fp = $this->open_archive()) {
            ...
            while ($block = fread($fp, 512)) {
                $temp = unpack("a100name/a8mode/a8uid/a8gid/a12size/a12mtime/a8checksum/a1type/a100symlink/a6magic/a2temp/a32temp/a32temp/a8temp/a8temp/a155backer/a12temp", $block);
                $file = array(
                    'name' => $temp['backer'] . $temp['name'],
                    'stat' => array(
                        2 => $temp['mode'],
                        4 => octdec($temp['uid']),
                        5 => octdec($temp['gid']),
