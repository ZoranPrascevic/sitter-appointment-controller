...
function edit_evaluation_form(invoker, id) {
    ...
    $('#evaluation_form select[name="sitterposition"]').val($(invoker).data('position'));
    $('#evaluation_form select[name="sitterposition"]').change();

    $.post(admin_url + 'appointment/get_list_criteria_edit/' + id).done(function (response) {
...
function group_criteria_change(invoker) {
    ...
    $.post(admin_url + 'appointment/get_criteria_by_group/' + invoker.value).done(function (response) {
        response = JSON.parse(response);
