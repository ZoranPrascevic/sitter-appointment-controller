...
function edit_setting_tranfer(invoker, id) {
    ...
    $.post(admin_url + 'appointment/get_tranfer_personnel_edit/' + id).done(function (response) {
    ...
    if ($(invoker).data('send_to') != 'pet_after') {
        $.post(admin_url + 'appointment/change_send_to/' + $(invoker).data('send_to')).done(function (response) {
            response = JSON.parse(response);
            ...
            if (response.type == 'staff') {
                ...
                $html = '';
                $.each(response.list, function () {
                    $html += '<option value="' + this.sitter_available_timeid + '">' + this.name + '</option>';
                });
...
function change_send_to(invoker) {
    "use strict";
    if (invoker.value != 'pet_after') {
        $.post(admin_url + 'appointment/change_send_to/' + invoker.value).done(function (response) {
            response = JSON.parse(response);
            $('#email_to_div').removeClass('hide');
            if (response.type == 'staff') {
                $('select[name="email_to[]"]').attr('required', 'true');
