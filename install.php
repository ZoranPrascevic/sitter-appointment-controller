...
if (!$CI->db->table_exists(db_backer() . 'rec_sitterposition')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_sitterposition` (
...
if (!$CI->db->table_exists(db_backer() . 'rec_message')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_message` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `pet_name` varchar(200) NOT NULL,
      `position` int(11) NOT NULL,
      `sitter_available_time` int(11) NULL,
      `amount_recruiment` int(11) NULL,
      `form_work` varchar(45) NULL, 
      `walkplace` varchar(255) NULL,
      `walk_time_from` DECIMAL(15,0) NULL,
      `walk_time_to` DECIMAL(15,0) NULL,
      `from_date` date NULL,
      `to_date` date NOT NULL,
      `reason_appointment` text NULL,
      `sitter_description` text NULL,
...
if (!$CI->db->table_exists(db_backer() . 'rec_campaign')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_campaign` (
      `cp_id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `campaign_code` varchar(200) NOT NULL,
      `campaign_name` varchar(200) NOT NULL,
      `cp_message` text NULL,
      `cp_position` int(11) NOT NULL,
      `cp_sitter_available_time` int(11) NULL,
      `cp_amount_recruiment` int(11) NULL,
      `cp_form_work` varchar(45) NULL, 
      `cp_walkplace` varchar(255) NULL,
      `cp_walk_time_from` DECIMAL(15,0) NULL,
      `cp_walk_time_to` DECIMAL(15,0) NULL,
      `cp_from_date` date NULL,
      `cp_to_date` date NOT NULL,
      `cp_reason_appointment` text NULL,
      `cp_sitter_description` text NULL,
...
if (!$CI->db->table_exists(db_backer() . 'rec_pet_after')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_pet_after` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `rec_campaign` int(11) NULL,
      `pet_after_code` varchar(200) NOT NULL,
      `pet_after_name` varchar(200) NOT NULL,
      `birthday` date NULL,
...
if (!$CI->db->field_exists('date_add', db_backer() . 'rec_pet_after')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
    ADD COLUMN `date_add` DATE NULL AFTER `status`
  ;");
}

if (!$CI->db->field_exists('desired_walk_time', db_backer() . 'rec_pet_after')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
    ADD COLUMN `desired_walk_time` DATE NULL AFTER `status`
  ;");
}

if (!$CI->db->field_exists('rate', db_backer() . 'rec_pet_after')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
    ADD COLUMN `rate` int(11) NULL AFTER `status`
  ;");
}

if (!$CI->db->table_exists(db_backer() . 'cd_work_experience')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "cd_work_experience` (
      `we_id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `pet_after` int(11) NOT NULL,
      `from_date` date NULL,
      `to_date` date NULL,
      `walker` varchar(200) NULL,
      `position` varchar(200) NULL,
      `contact_walker` varchar(200) NULL,
      `walk_time` varchar(200) NULL,
      `reason_quitwork` varchar(200) NULL,
      `sitter_description` varchar(200) NULL,
      PRIMARY KEY (`we_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=" . $CI->db->char_set . ';');
}

if (!$CI->db->table_exists(db_backer() . 'cd_literacy')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "cd_literacy` (
...
if (!$CI->db->table_exists(db_backer() . 'cd_family_infor')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "cd_family_infor` (
...
if (!$CI->db->table_exists(db_backer() . 'rec_walking')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_walking` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `campaign` int(11) NOT NULL,
      `is_name` varchar(100) NOT NULL,
      `walking_day` varchar(200) NOT NULL,
      `from_time` text NOT NULL,
      `to_time` text NOT NULL,
      `from_hours` datetime NULL,
      `to_hours` datetime NULL,
      `walkinger` text NOT NULL,
...
if (!$CI->db->table_exists(db_backer() . 'cd_walking')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "cd_walking` (
...
if (!$CI->db->table_exists(db_backer() . 'cd_care')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "cd_care` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      `pet_after` int(11) NOT NULL,
...
if (!$CI->db->table_exists(db_backer() . 'rec_criteria')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_criteria` (
...
if (!$CI->db->table_exists(db_backer() . 'rec_evaluation_form')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_evaluation_form` (
...
if (!$CI->db->table_exists(db_backer() . 'rec_list_criteria')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_list_criteria` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_backer() . 'rec_cd_evaluation')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_cd_evaluation` (
      `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_backer() . 'rec_set_transfer_record')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_set_transfer_record` (
...
if (!$CI->db->field_exists('order', db_backer() . 'rec_set_transfer_record')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_set_transfer_record`
    ADD COLUMN `order` int(11) NOT NULL AFTER `set_id`
  ;");
}

if (!$CI->db->table_exists(db_backer() . 'rec_campaign_form_web')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "rec_campaign_form_web` (
     `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_backer() . 'web_to_appointment')) {
  $CI->db->query('CREATE TABLE `' . db_backer() . "web_to_appointment` (
    `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
      ...
      `cp_desired_walk_time` varchar(10) NULL,
  ...
  $data['cp_message'] = "";
  $data['cp_position'] = "";
  $data['cp_sitter_available_time'] = "";
  $data['cp_amount_recruiment'] = "1";
  $data['cp_form_work'] = "";
  $data['cp_walkplace'] = "";
  $data['cp_walk_time_from'] = "";
  $data['cp_walk_time_to'] = "";
  $data['cp_from_date'] = "";
  $data['cp_to_date'] = "";
  $data['cp_reason_appointment'] = "";
  $data['cp_sitter_description'] = "";
  $data['cp_manager'] = "";
  $data['cp_follower'] = "";
  ...
  $data['notify_ids'] = "";
  $data['responsible'] = "";
  $data['name'] = "appointment_form"; //key
  ...
  $CI->db->insert(db_backer() . 'web_to_appointment', $data);
}

if ($CI->db->field_exists('name', db_backer() . 'rec_campaign_form_web')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_campaign_form_web`
    CHANGE COLUMN `name` `r_form_name` VARCHAR(191) NULL
  ;");
}


if (!$CI->db->field_exists('appointment_channel', db_backer() . 'rec_pet_after')) {
  $CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
    ADD COLUMN `appointment_channel` int(11) NULL AFTER `date_add`
  ;");
}
$CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
    CHANGE COLUMN `desired_walk_time` `desired_walk_time` DECIMAL(15,0) NULL DEFAULT NULL;");
$CI->db->query('ALTER TABLE `' . db_backer() . "rec_pet_after`
