...
class Forms extends ClientsController
{
    ...
    public function wtl($key)
    {
        ...
        $form = $this->appointment_model->get_form([
            'form_key' => $key,
        ]);
            ...
            if ($this->input->post('key') == $key) {
                ...
                foreach ($post_data as $name => $val) {
                    if (strpos($name, 'form-cf-') !== false) {
                        ...
                        if ($this->db->field_exists($name, db_backer() . 'leads')) {
                            if ($name == 'country') {
                                if (!is_numeric($val)) {
                                    if ($val == '') {
                                        $val = 0;
                                        ...
                                        $country = $this->db->get(db_backer() . 'countries')->row();
                                        if ($country) {
                ...
                if ($form->allow_duplicate == 0) {
                    ...
                    if (count($where) > 0) {
                        $total = total_rows(db_backer() . 'leads', $where);
                        ...
                        if ($total == 1) {
                            $this->db->where($where);
                            $duplicateLead = $this->db->get(db_backer() . 'leads')->row();
                        }
                        ...
                        if ($total > 0) {
                            ...
                            if ($form->create_task_on_duplicate == 1) {
                                ...
                                } elseif (isset($regular_fields['walker'])) {
                                    $task_name = $regular_fields['walker'];
                                } else {
                                ...
                                $this->db->insert(db_backer() . 'tasks', $task_data);
                ...
                if ($insert_to_db == true) {
                    ...
                    $this->db->insert(db_backer() . 'leads', $regular_fields);
                    ...
                    if ($lead_id) {
                        ...
                        if ($form->notify_lead_imported != 0) {
                            ...
                            if ($to_responsible == false && is_array($ids) && count($ids) > 0) {
                                ...
                                $staff = $this->db->get(db_backer() . 'staff')->result_array();
                            } else {
                                $staff = [
                                    [
        ...
        $data['form'] = $form;
        $this->load->view('forms/appointment_channel_form', $data);
    }
    ...
    public function ticket()
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $required = ['subject', 'sitter_available_time', 'email', 'name', 'message', 'priority'];
            ...
            $post_data = [
                ...
                'sitter_available_time' => $post_data['sitter_available_time'],
            ...
            $result = $this->db->get(db_backer() . 'contacts')->row();
